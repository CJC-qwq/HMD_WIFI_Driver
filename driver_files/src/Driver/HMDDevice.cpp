...
ExampleDriver::HMDDevice::HMDDevice(std::string serial):serial_(serial)
{
    ...
    bind(udpSocket, (sockaddr*)&remoteAddr, sizeof(remoteAddr));
    // 设置初始姿态
    last_pose_ = IVRDevice::MakeDefaultPose();

    // 设置初始位置 (单位是米)
    last_pose_.vecPosition[0] = 0.0f;  // X 位置
    last_pose_.vecPosition[1] = 0.0f;  // Y 位置 (通常是头部高度，约1.7米)
    last_pose_.vecPosition[2] = 0.0f;  // Z 位置

    // 设置初始旋转 (四元数)
    last_pose_.qRotation.w = 1.0f;  // 默认朝向
    last_pose_.qRotation.x = 0.0f;
    last_pose_.qRotation.y = 0.0f;
    last_pose_.qRotation.z = 0.0f;

    // 设置姿态有效
    last_pose_.poseIsValid = true;
...
void ExampleDriver::HMDDevice::Update()
{
   /*if (this->device_index_ == vr::k_unTrackedDeviceIndexInvalid)
    ...
    int n = recvfrom(udpSocket, udpBuffer, sizeof(udpBuffer) - 1, 0, (sockaddr*)&remoteAddr, &len);
    if (n > 0) {
        udpBuffer[n] = 0;
        float w, x, y, z;
        if (sscanf(udpBuffer, "%f,%f,%f,%f", &w, &x, &y, &z) == 4) {
            // 更新last_pose_的四元数
            last_pose_.qRotation.w = w;
            last_pose_.qRotation.x = x;
            last_pose_.qRotation.y = y;
            last_pose_.qRotation.z = z;
            last_pose_.poseIsValid = true;
            last_pose_.deviceIsConnected = true;
            last_pose_.result = vr::TrackingResult_Running_OK;
        }
    ...
    // 上报给SteamVR
    vr::VRServerDriverHost()->TrackedDevicePoseUpdated(this->GetDeviceIndex(), last_pose_, sizeof(last_pose_));
}
